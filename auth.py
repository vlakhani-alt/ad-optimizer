...
Auth methods:
  ...
  2. Email/password via streamlit-authenticator

Auth is OPTIONAL — if no auth secrets are configured, the app runs without login
(all users get super_admin level). This lets local dev work without auth setup.
...
[auth.roles]
# Map emails to roles. Users not listed default to "user" role.
super_admins = ["admin@agency.com"]
admins = ["manager@agency.com", "lead@agency.com"]
...
def _get_user_role(email: str) -> str:
    """Get role for an email address. Checks secrets, then users file."""
    if not email:
        return "user"
...
def _has_auth_config() -> bool:
    """Check if any auth configuration exists in secrets."""
    try:
...
def _render_login_page() -> bool:
    ...
    if _password_auth_available():
        try:
            ...
            st.warning("Email login not available (streamlit-authenticator not installed)")
...
def check_auth() -> bool:
    ...
    If no auth is configured, always returns True (open access).
    """
...
def get_current_role() -> str:
    ...
    Returns 'super_admin' if no auth is configured (open access mode).
    """
    if not _has_auth_config():
        return "super_admin"
...
def render_logout_button():
    ...
    if st.session_state.get("authentication_status") and _password_auth_available():
...
def add_user(email: str, name: str, role: str = "user"):
    """Add a user (stored in users.json)."""
    users = _load_users()
...
def render_user_management():
    ...
    if st.button("Add User", key="admin_add_user") and _new_email:
        add_user(_new_email, _new_name or _new_email, _new_role)
        st.success(f"Added {_new_email} as {ROLES[_new_role]['label']}")
        st.rerun()
